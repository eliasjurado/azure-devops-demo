...
namespace Dojo.Bakery.Inventory.Domain.Entities
{
    public class Inventory: AggregateRoot
    {
        ...
        public Inventory(Guid productId, Guid storeId, int stock)
        {
            ...
            DomainExceptionValidation.When(productId == Guid.Empty, $"{nameof(productId)} cannot be empty");
            DomainExceptionValidation.When(storeId == Guid.Empty, $"{nameof(storeId)} cannot be empty");
            ProductId = productId;  
            StoreId = storeId;  
            Stock = stock;
